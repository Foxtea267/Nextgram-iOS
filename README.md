...
Nextgram 并不是 AyuGram 的官方 iOS 版本，与 AyuGram 项目不存在官方隶属、授权或背书关系。
...
Nextgram 自身的增强代码应尽可能与 Telegram 上游代码分离。

必须修改 Telegram 上游代码时，建议使用：

// MARK: NEXTGRAM

作为修改锚点，方便后续同步 Telegram-iOS 上游。

每次 rebase 或 checkout Telegram-iOS 上游后，请先同步 Submodule：

git submodule update --init --recursive
git submodule status --recursive

确认 Submodule 状态中没有 +、- 或 U 前缀。

Build

Nextgram-iOS 使用 Telegram-iOS 原有的 Bazel 构建系统。
...
完整签名环境下，应保持 Telegram 的所有扩展启用，包括：
