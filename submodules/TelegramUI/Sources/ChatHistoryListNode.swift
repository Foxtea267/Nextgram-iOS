...
public final class ChatHistoryListNodeImpl: ASDisplayNode, ChatHistoryNode, ChatHistoryListNode {
    ...
    public init(
        ...
        self.unseenReactionsProcessingManager.process = { [weak self] messageIds in
            ...
            if strongSelf.canReadHistoryValue && !strongSelf.suspendReadingReactions && !strongSelf.context.sharedContext.immediateExperimentalUISettings.skipReadHistory && !NagramSettings.shared.suppressReadReceipts { // MARK: NAGRAM // MARK: NEXTGRAM
    ...
    private func attemptReadingReactions() {
        if self.canReadHistoryValue && !self.suspendReadingReactions && !self.context.sharedContext.immediateExperimentalUISettings.skipReadHistory && !NagramSettings.shared.suppressReadReceipts && !self.messageIdsWithReactionsScheduledForMarkAsSeen.isEmpty { // MARK: NAGRAM // MARK: NEXTGRAM
    ...
    private func processDisplayedItemRangeChanged(displayedRange: ListViewDisplayedItemRange, transactionState: ChatHistoryTransactionOpaqueState) {
        ...
        if let visible = displayedRange.visibleRange {
            ...
            if !messageIdsWithUnseenReactions.isEmpty {
                ...
                if self.canReadHistoryValue && !self.context.sharedContext.immediateExperimentalUISettings.skipReadHistory && !NagramSettings.shared.suppressReadReceipts { // MARK: NAGRAM // MARK: NEXTGRAM
    ...
    private func updateReadHistoryActions() {
        ...
        if canRead != (self.interactiveReadActionDisposable != nil) {
            if let interactiveReadActionDisposable = self.interactiveReadActionDisposable {
                ...
                if !self.context.sharedContext.immediateExperimentalUISettings.skipReadHistory && !NagramSettings.shared.suppressReadReceipts && !self.context.account.isSupportUser { // MARK: NAGRAM // MARK: NEXTGRAM
                    if case let .peer(peerId) = self.chatLocation {
        ...
        if canRead != (self.interactiveReadReactionsDisposable != nil) {
            if let interactiveReadReactionsDisposable = self.interactiveReadReactionsDisposable {
                ...
                if case let .peer(peerId) = self.chatLocation {
                    if !self.context.sharedContext.immediateExperimentalUISettings.skipReadHistory && !NagramSettings.shared.suppressReadReceipts { // MARK: NAGRAM // MARK: NEXTGRAM
