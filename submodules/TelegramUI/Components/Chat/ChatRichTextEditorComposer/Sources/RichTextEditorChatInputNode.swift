...
public final class RichTextEditorChatInputNode: ASDisplayNode, ChatRichTextInputNode {
    ...
    // MARK: NAGRAM — Software Return can be consumed by the chat panel's send-with-Return setting.
    public var onSoftwareReturn: (() -> Bool)? {
        didSet {
            self.editorView.onSoftwareReturn = self.onSoftwareReturn
        }
    }
    ...
    // MARK: NAGRAM
    public var inputReturnKeyType: UIReturnKeyType {
        get { self.editorView.inputReturnKeyType }
        set { self.editorView.inputReturnKeyType = newValue }
    }
